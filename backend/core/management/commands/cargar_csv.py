...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, row in df.iterrows():
            ...
            actividad_consolidada_nombre_original = limpiar(
                row.get("Actividad consolidada Original")
            )
            linea_proyecto_nombre_original = limpiar(
                row.get("Linea del Proyecto Original")
            )
            ...
            facultad_nombre_original = limpiar(
                row.get("Facultad/Instituto/Área Original")
            )
            escuela_nombre_original = limpiar(
                row.get("Escuela/Programa Académico Original")
            )
            ...
            estrategia_nombre_original = limpiar(
                row.get("Linea de estrategia Original")
            )
            ...
            if nombre_persona:
                identificadores_fila += 1
            if tipo_documento:
                identificadores_fila += 1
            if sexo:
                identificadores_fila += 1
            if parse_int(edad) is not None:
                identificadores_fila += 1
            if telefono_int:
                identificadores_fila += 1
            if estamento:
            ...
            if (
                not persona_obj
                and not numero_documento_int
                and not correo_normalizado
                and identificadores_fila >= 2
            ):
                        ...
                        if p.nombre and nombre_persona:
                            ...
                            if (
                                fuzz.token_sort_ratio(
                                    p.nombre.lower(), nombre_persona.lower()
                                )
                                >= 85
                            ):
                                campos_coinciden += 1
            ...
            if persona_obj:
                ...
                if nombre_persona_original and (
                    not persona_obj.nombre_original
                    or persona_obj.nombre_original.strip() == ""
                ):
                    persona_obj.nombre_original = nombre_persona_original
                if tipo_documento and (
                    not persona_obj.tipo_documento
                    or persona_obj.tipo_documento.strip() == ""
                ):
                    persona_obj.tipo_documento = tipo_documento
                if tipo_documento_original and (
                    not persona_obj.tipo_documento_original
                    or persona_obj.tipo_documento_original.strip() == ""
                ):
                ...
                if correo_normalizado and not persona_obj.correo:
                    conflicto = (
                        Persona.objects.filter(correo__iexact=correo_normalizado)
                        .exclude(pk=persona_obj.pk)
                        .exists()
                    )
                    if not conflicto:
                        persona_obj.correo = correo_normalizado
                if estamento and (
                    not persona_obj.estamento or persona_obj.estamento.strip() == ""
                ):
                    persona_obj.estamento = estamento
                # guardar estamento original si viene
                if estamento_original and (
                    not getattr(persona_obj, "estamento_original", None)
                ):
                    try:
                ...
                if numero_documento_int and not persona_obj.numero_documento:
                    conflicto_doc = (
                        Persona.objects.filter(numero_documento=numero_documento_int)
                        .exclude(pk=persona_obj.pk)
                        .exists()
                    )
            ...
            if semestre_int is not None:
                try:
                    ...
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"No se pudo crear/actualizar Estudiante para persona {persona_obj.pk} (fila {i+1})"
                            )
                        )
            ...
            actividad_obj, _ = Actividad.objects.get_or_create(
                nombre=actividad_nombre, defaults=actividad_defaults
            )
            ...
            if actividad_consolidada_nombre:
                ...
                if actividad_consolidada_nombre_original:
                    act_con_defaults["nombre_original"] = (
                        actividad_consolidada_nombre_original
                    )
                act_con_obj, _ = ActividadConsolidada.objects.get_or_create(
                    nombre=actividad_consolidada_nombre, defaults=act_con_defaults
                )
                Consolidacion.objects.get_or_create(
                    actividad=actividad_obj, actividad_consolidada=act_con_obj
                )
            ...
            if linea_proyecto_nombre:
                ...
                if linea_proyecto_nombre_original:
                    linea_proj_defaults["nombre_original"] = (
                        linea_proyecto_nombre_original
                    )
                linea_proj_obj, _ = LineaProyecto.objects.get_or_create(
                    nombre=linea_proyecto_nombre, defaults=linea_proj_defaults
                )
                AsociacionProyecto.objects.get_or_create(
                    actividad=actividad_obj, linea_proyecto=linea_proj_obj
                )
            ...
            if prioridad_nombre:
                ...
                prioridad_obj, _ = Prioridad.objects.get_or_create(
                    nombre=prioridad_nombre, defaults=prioridad_defaults
                )
                PrioridadAsociada.objects.get_or_create(
                    actividad=actividad_obj, prioridad=prioridad_obj
                )
            ...
            if estrategia_nombre:
                ...
                est_obj, _ = LineaEstrategia.objects.get_or_create(
                    nombre=estrategia_nombre, defaults=est_defaults
                )
                EstrategiaAsociada.objects.get_or_create(
                    actividad=actividad_obj, linea_estrategia=est_obj
                )
